...
async function fetchKPIs(dept: string | null): Promise<PortfolioKPIs> {
  const sql = `WITH deduped AS (
    ...
    FROM ${SCHEMA}.FACT_DEMAND_RISK
    WHERE IS_CURRENT_WEEK = TRUE${deptWhere(dept)}
...
async function fetchDrivers(dept: string | null): Promise<DriverRow[]> {
  const sql = `SELECT d.DEPARTMENT, d.CATEGORY_L3, p.CATEGORY_L2, d.DRIVER_NAME,
    AVG(d.CONTRIBUTION_PP) AS CONTRIBUTION_PP,
    AVG(CASE WHEN d.CONFIDENCE_SCORE > 0 THEN d.CONFIDENCE_SCORE END) AS CONFIDENCE_SCORE,
    MAX(d.IS_SIGNIFICANT::INT)::BOOLEAN AS IS_SIGNIFICANT,
    MAX(d.MULTICOLLINEARITY_FLAG::INT)::BOOLEAN AS MULTICOLLINEARITY_FLAG,
    AVG(d.TOTAL_DEVIATION_PP) AS TOTAL_DEVIATION_PP
  FROM ${SCHEMA}.FACT_DRIVER_ATTRIBUTION d
  JOIN (SELECT DISTINCT CATEGORY_L3, CATEGORY_L2 FROM ${SCHEMA}.DIM_PRODUCT) p
    ON d.CATEGORY_L3 = p.CATEGORY_L3
  WHERE d.SCOPE_ID = 'baseline'${deptWhere(dept, "d")}
  GROUP BY d.DEPARTMENT, d.CATEGORY_L3, p.CATEGORY_L2, d.DRIVER_NAME
  ORDER BY d.DEPARTMENT, ABS(AVG(d.CONTRIBUTION_PP)) DESC`;
  const rows = parseRows(await executeStatement(sql));
  return rows.map((r) => ({
    department: str(r.DEPARTMENT),
    category: str(r.CATEGORY_L3),
